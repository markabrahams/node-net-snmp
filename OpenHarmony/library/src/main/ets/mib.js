var MIB = function (dir) {
	...
	var newMIB = ({
		...
		Import: function (FileName, resourceManager) {
			try {
				let content = resourceManager.getRawFileContentSync(FileName);
				let contents = Array.from(content).map(charCode => String.fromCharCode(charCode)).join('');
				this.ParseModule(FileName.split('.')[0], contents);
			} catch (error) {
				console.error('SNMP-- callback getRawFileContent failed ' + JSON.stringify(error));
			}
		},
		...
		ParseChar: function (FileName, char, row, column) {
			switch (char) {
				...
				case '(':
					...
					if (this.CharBuffer.isComment || ((this.CharBuffer.isOID || this.CharBuffer.nested > 0) &&
				...
				if (this.SymbolBuffer.hasOwnProperty(ModuleName)) {
								...
								if (isObjectIdentifierAssignment || isTrapTypeDefinition) {
									...
									if (Symbols[i - 1] == 'OBJECT IDENTIFIER') {
										...
										Object[Symbols[i - 2]]['OBJECT IDENTIFIER'] =
											Symbols[i + 1].replace("{", "").replace("}", "").trim().replace(/\s+/, " ");
										if (Object[Symbols[i - 2]]['OBJECT IDENTIFIER'] == '0 0') {
											...
											const { oidString, nameString, unresolvedObject } =
												this.getOidAndNamePaths(Object[Symbols[i - 2]]['OBJECT IDENTIFIER'],
													Symbols[i - 2], ModuleName);
										...
										if (isObjectIdentifierAssignment) {
											Object[Symbols[macroIndex - 1]]['OBJECT IDENTIFIER'] =
												Symbols[i + 1].replace("{", "").replace("}", "").trim()
													.replace(/\s+/, " ");
											if (Object[Symbols[macroIndex - 1]]['OBJECT IDENTIFIER'] == '0 0') {
												...
												const { oidString, nameString, unresolvedObject } =
													this.getOidAndNamePaths(Object[Symbols[macroIndex -
														1]]['OBJECT IDENTIFIER'], Symbols[macroIndex - 1], ModuleName);
					...
					if (unresolvedObjects.length > 0) {
						for (const unresolved of unresolvedObjects) {
							...
							const { oidString, nameString, unresolvedObject } =
								this.getOidAndNamePaths(obj['OBJECT IDENTIFIER'], unresolved, ModuleName);
		...
		GetRanges: function (mibRanges) {
			let rangesString = mibRanges.replace(/ */g, '')
				.replace(/\(SIZE/, '')
				.replace(/\)/, '')
				.replace(/\(/, '')
				.replace(/\)/, '');
			...
			if (parent == 'iso') {
				...
				if (!parentObject) {
					...
					for (let importModule of importModules) {
						if (this.Modules[ModuleName]['IMPORTS'][importModule][parent]) {
							parentObject = this.Modules[ModuleName]['IMPORTS'][importModule][parent];
							break;
						}
					}
				}
