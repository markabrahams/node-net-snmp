...
import GlobalContext from './src/main/ets/GlobalContext';
...
var readPdu = function (reader, scoped) {
	...
	var contextName;
	if (scoped) {
		reader = new ber.BerReader(reader.readString(ber.ASN1.Sequence | ber.ASN1.Constructor, true));
		contextEngineID = reader.readString(ber.ASN1.OctetString, true);
...
Message.prototype.toBufferV3 = function () {
	...
	// ScopedPDU
	var scopedPduWriter = new ber.BerWriter();
	scopedPduWriter.startSequence();
	var contextEngineID =
		this.pdu.contextEngineID ? this.pdu.contextEngineID : this.msgSecurityParameters.msgAuthoritativeEngineID;
	if (contextEngineID.length == 0) {
		scopedPduWriter.writeString("");
	} else {
		scopedPduWriter.writeBuffer(contextEngineID, ber.ASN1.OctetString);
	}
	...
	writer.writeByte(ber.ASN1.OctetString);
	writer.writeByte(1);
	...
	writer.startSequence(ber.ASN1.OctetString);
	writer.startSequence();
	...
	if (this.msgSecurityParameters.msgAuthoritativeEngineID.length == 0) {
		...
		writer.writeBuffer(this.msgSecurityParameters.msgAuthoritativeEngineID, ber.ASN1.OctetString);
	}
...
Message.createFromBuffer = function (buffer, user) {
	...
	if (message.version != 3) {
		...
		message.msgGlobalData.msgFlags = reader.readString(ber.ASN1.OctetString, true)[0];
		message.msgGlobalData.msgSecurityModel = readInt32(reader);

		// msgSecurityParameters
		message.msgSecurityParameters = {};
		var msgSecurityParametersReader = new ber.BerReader(reader.readString(ber.ASN1.OctetString, true));
	...
	if (message.version != req.message.version) {
		...
		if (this.proxy) {
			...
			if (!req.originalPdu || !req.allowReport) {
				if (Array.isArray(message.pdu.varbinds) && message.pdu.varbinds[0] &&
...
Session.prototype.registerRequest = function (req) {
	...
	req.timer = setTimeout(function () {
		if (req.retries-- > 0) {
			console.info('SNMP--> retries send');
			me.send(req);
		} else {
...
Session.prototype.simpleGet = function (pduClass, feedCb, varbinds,
	...
	if (this.version == Version3) {
		...
		console.info('SNMP--> simpleGet' + JSON.stringify(req));
		this.send(req);
	}
};
...
Session.create = function (target, community, options) {
	...
	if (version != Version1 && version != Version2c) {
		throw new ResponseInvalidError("SNMP community session requested but version '" + options.version +
...
Session.createV3 = function (target, user, options) {
	...
	if (options && options.version && options.version != Version3) {
		throw new ResponseInvalidError("SNMPv3 session requested but version '" + options.version +
...
Listener.prototype.startListening = function () {
	var me = this;
	this.sockets = {};
	console.info("SNMP-->startListening")
	for (const socketOptions of this.socketOptions) {
		console.info("SNMP-->socketOptions   " + JSON.stringify(socketOptions))
...
Listener.processIncoming = function (buffer, authorizer, callback) {
	...
	if (message.version == Version3) {
		...
		if ((message.user.level == SecurityLevel.authNoPriv || message.user.level == SecurityLevel.authPriv) &&
		...
		if (!community && !authorizer.disableAuthorization) {
			callback(new RequestFailedError("Local community not found for message with community " +
			message.community));
			return;
		}
	}
...
Receiver.prototype.onMsg = function (socket, buffer, rinfo) {
	...
	if (message.pdu.type == PduType.Trap || message.pdu.type == PduType.TrapV2) {
		...
		this.callback(new RequestInvalidError("Unexpected PDU type " + message.pdu.type + " (" +
		PduType[message.pdu.type] + ")"));
	}
};
...
ModuleStore.prototype.loadFromFile = function (fileName, resManager) {
	var modulesBeforeLoad = this.getModuleNames();
	this.parser.Import(fileName, resManager);
	this.parser.Serialize();
	var modulesAfterLoad = this.getModuleNames();
	var newModulesForTranslation =
		modulesAfterLoad.filter(moduleName => modulesBeforeLoad.indexOf(moduleName) === -1);
...
ModuleStore.prototype.getProvidersForModule = function (moduleName) {
	...
	for (var i = 0; i < entryArray.length; i++) {
		...
		var maxAccess = (typeof mibEntry["MAX-ACCESS"] != "undefined" ? mibEntry["MAX-ACCESS"] :
			(access ? AccessToMaxAccess[access] : "not-accessible"));
		...
		if (syntax) {
			...
			if (syntax.startsWith("SEQUENCE OF")) {
				...
				while (currentTableProvider || i >= entryArray.length) {
					...
					maxAccess = (typeof mibEntry["MAX-ACCESS"] != "undefined" ? mibEntry["MAX-ACCESS"] :
						(access ? AccessToMaxAccess[access] : "not-accessible"));
...
ModuleStore.prototype.loadBaseModules = function (resourceManager) {
	for (var mibModule of ModuleStore.BASE_MODULES) {
		console.info("SNMP-->mibModule :" + mibModule)
		this.parser.Import(mibModule + ".mib", resourceManager);
	}
	console.info("SNMP-->Import end")
	this.parser.Serialize();
	console.info("SNMP-->Serialize end")
...
ModuleStore.prototype.translate = function (name, destinationFormat) {
	...
	if (sourceFormat === lowercaseDestinationFormat) {
		...
		var capitalizedDestinationFormat =
			destinationFormat.charAt(0).toUpperCase() + destinationFormat.slice(1).toLowerCase();
...
ModuleStore.create = function (resourceManager) {
	var store = new ModuleStore();
	store.loadBaseModules(resourceManager);
	return store;
};
...
Mib.prototype.dumpProviders = function () {
	...
	for (var provider of Object.values(this.providers)) {
		extraInfo = provider.type == MibProviderType.Scalar ? ObjectType[provider.scalarType] :
			"Columns = " + provider.tableColumns.length;
		console.log(MibProviderType[provider.type] + ": " + provider.name + " (" + provider.oid + "): " +
			extraInfo);
	}
};
...
Mib.prototype.setScalarValue = function (scalarName, newValue) {
	...
	var instanceNode;
	var provider;
	console.info("SNMP-- providers" + JSON.stringify(this.provider))
...
Mib.prototype.getTableRowInstanceFromRow = function (provider, row) {
	...
	for (var localColumnPart of localColumnParts) {
		localColumnPosition =
			provider.tableColumns.findIndex(column => column.number == localColumnPart.columnNumber);
		oidArrayForValue =
			this.getOidAddressFromValue(row[foreignColumnParts.length + localColumnPosition], localColumnPart);
		rowIndex = rowIndex.concat(oidArrayForValue);
	}
	return rowIndex;
};
...
Agent.prototype.isAllowed = function (pduType, provider, instanceNode) {
	...
	if (provider.type === MibProviderType.Scalar) {
		...
		maxAccess =
			columnEntry ? columnEntry.maxAccess || MaxAccess['not-accessible'] : MaxAccess['not-accessible'];
	}
	...
	for (let i = 0; i < requestPdu.varbinds.length; i++) {
		...
		if (!instanceNode) {
			...
			if (!providerNode || instanceNode.value === undefined) {
				...
				!this.authorizer.getAccessControlModel()
				...
				requestPdu.varbinds[i].requestValue =
					this.castSetValue(requestPdu.varbinds[i].type, requestPdu.varbinds[i].value);
		...
		(function (savedIndex) {
			...
			mibRequests[savedIndex].done = function (error) {
				...
				if (error) {
					if ((typeof responsePdu.errorStatus == "undefined" ||
					...
					let subOid;
					let subAddr;
					if (providerNode && providerNode.provider &&
						providerNode.provider.type == MibProviderType.Table) {
						column = instanceNode.getTableColumnFromInstanceNode();
					...
					if (requestPdu.type == PduType.SetRequest) {
						...
						let rowStatusColumn = provider.type == MibProviderType.Table
							?
						provider.tableColumns.reduce((acc, current) => current.rowStatus ? current.number : acc,
							null)
							: null;
				...
				if (requestPdu.type == PduType.SetRequest) {
					...
					if (requestPdu.varbinds[savedIndex].requestValue) {
						responseVarbind.requestValue = me.castSetValue(requestPdu.varbinds[savedIndex].type,
				...
				if (providerNode && providerNode.provider.type == MibProviderType.Table) {
					...
					responseVarbind.columnPosition =
						providerNode.provider.tableColumns.findIndex(tc => tc.number == column);
...
let createSession = Session.create
let createV3Session = Session.createV3;

let createReceiver = Receiver.create;
let createAgent = Agent.create;
let createModuleStore = ModuleStore.create;
let createSubagent = Subagent.create;
let createMib = Mib.create;
